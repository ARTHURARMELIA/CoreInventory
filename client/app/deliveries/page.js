...
export default function DeliveriesPage() {
  ...
  const [form, setForm] = useState({ customer: '', notes: '', lines: [{ product: '', warehouse: '', quantity: 1 }] });
  ...
  const load = () => {
    ...
    deliveryApi.list(params).then(setList).catch(() => setList([])).finally(() => setLoading(false));
  };
  useEffect(() => {
  ...
  const save = async (e) => {
    ...
    setSaving(true);
    try {
      await deliveryApi.create({ customer: form.customer, notes: form.notes, lines });
      setModal(false);
      setForm({ customer: '', notes: '', lines: [{ product: '', warehouse: '', quantity: 1 }] });
      load();
    } catch (err) {
  ...
  const columns = [
    { id: 'deliveryNumber', header: 'Number', render: (r) => <span className="font-mono text-sky-400">{r.deliveryNumber}</span> },
    ...
    <div className="p-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => { setModal(true); setError(''); }} className="rounded-lg bg-sky-600 px-4 py-2 text-sm font-medium text-white hover:bg-sky-500">New delivery</button>
        </div>
      </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="New delivery order">
        <form onSubmit={save} className="space-y-4">
          ...
          <div>
            ...
            <input required value={form.customer} onChange={(e) => setForm((f) => ({ ...f, customer: e.target.value }))} className="w-full rounded-lg border border-slate-600 bg-slate-800 px-3 py-2 text-slate-100" />
          </div>
          <div>
